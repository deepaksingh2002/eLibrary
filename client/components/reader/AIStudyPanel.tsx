...
export default function AIStudyPanel({ bookId, bookTitle }: Props) {
  ...
  const tabOrder: AIStudyTab[] = ["mcq", "keypoints"]
  ...
  return (
    <>
      ...
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/15 backdrop-blur" aria-hidden="true">
      ...
      <div
        className={`fixed right-0 top-0 z-10000 flex h-dvh w-full max-w-none flex-col overflow-hidden bg-slate-50 shadow-2xl transition-transform duration-300 ease-out dark:bg-slate-950 sm:w-[92vw] md:w-[82vw] lg:w-[72vw] xl:w-[60vw] ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="border-b border-slate-200 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.18),transparent_34%),linear-gradient(135deg,#2563eb,#4f46e5_55%,#0f172a)] px-4 py-4 text-white dark:border-slate-800 sm:px-5 sm:py-5">
          ...
          <div
            ...
            className="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-white/10 p-1 backdrop-blur"
          >
            {tabs.map((tab) => (
              <button
                key={tab.key}
        ...
        <div className="relative min-h-0 flex-1 overflow-y-auto bg-[linear-gradient(180deg,rgba(248,250,252,0.92),rgba(241,245,249,0.96))] p-4 sm:p-5 dark:bg-slate-950">
          <button
            type="button"
            onClick={goToPreviousTab}
            aria-label="Previous tab"
            className="absolute left-2 top-1/2 z-20 hidden h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white/90 text-slate-700 shadow-lg shadow-slate-900/10 backdrop-blur transition-all hover:bg-white hover:scale-105 dark:border-slate-800 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:bg-slate-900 sm:flex"
          >
          ...
          <button
            ...
            onClick={goToNextTab}
            aria-label="Next tab"
            className="absolute right-2 top-1/2 z-20 hidden h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white/90 text-slate-700 shadow-lg shadow-slate-900/10 backdrop-blur transition-all hover:bg-white hover:scale-105 dark:border-slate-800 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:bg-slate-900 sm:flex"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 sm:px-5">
              ...
              {chapterOptions.length > 0 && (
                <div className="mt-3 flex flex-col gap-1.5 border-t border-slate-100 pt-3 dark:border-slate-800">
                  <label htmlFor="chapter-select" className="text-xs font-semibold text-slate-700 dark:text-slate-200">
                    Focus Chapter
                  </label>
                  <select
                    id="chapter-select"
                    ...
                    className="w-full max-w-md rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 shadow-xs outline-hidden transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:focus:bg-slate-950"
                  >
