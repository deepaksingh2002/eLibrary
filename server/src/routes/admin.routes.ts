...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const books = await Book.find({ status: "published", isDeleted: false })
      ...
      .limit(limit)
      .select(
        "title author genre coverUrl downloads avgRating totalReviews createdAt",
      );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sort =
      sortOptions[(req.query.sort as AdminListSortKey) ?? "newest"] ??
      sortOptions.newest;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (books.length > 100)
      throw new ApiError(400, "Maximum 100 items per import");
    ...
    if (validBooks.length > 0) {
      try {
        ...
        if (
          ...
          for (const writeError of error.writeErrors as Array<{
            index: number;
            errmsg: string;
          }>) {
            ...
            errors.push({
              ...
              title:
                typeof failedBook?.title === "string"
                  ? failedBook.title
                  : "Unknown",
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const books = await Book.find({ isDeleted: false })
      .select(
        "title author genre language downloads avgRating totalReviews status createdAt",
      )
      .lean();
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="elibrary-books-${new Date().toISOString().slice(0, 10)}.csv"`,
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="elibrary-users-${new Date().toISOString().slice(0, 10)}.csv"`,
    );
    res.status(200).send(`\uFEFF${csvRows.join("\n")}`);
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sort =
      sortMap[(req.query.sort as AdminUserSortKey) ?? "newest"] ??
      sortMap.newest;
    ...
    const [users, total] = await Promise.all([
      User.find(filter)
        ...
        .limit(limit)
        .select(
          "name email role avatar streak totalBooksRead monthlyGoal createdAt lastActiveDate isDeleted",
        ),
