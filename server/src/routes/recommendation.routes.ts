...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    let rec = await Recommendation.findOne({ userId })
      .populate(
        "books.bookId",
        "title author coverUrl genre avgRating totalReviews downloads tags",
      )
      .lean();

    if (!rec) {
      console.log(
        "[Recommendations] No recommendations for user:",
        userId,
        "- computing cold start",
      );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res
      .status(200)
      .json({ similar: populated.filter((item) => item.book !== null) });
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      rec &&
      rec.computedAt &&
      Date.now() - rec.computedAt.getTime() < 3600000
    ) {
    ...
    res
      .status(200)
      .json({ message: "Recommendations refreshed", computedAt: new Date() });
  }),
);
