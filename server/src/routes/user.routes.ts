...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new ApiError(
        400,
        "monthlyGoal must be an integer between 0 and 100",
      );
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (name !== undefined) {
      if (
        ...
        throw new ApiError(
          400,
          "Name must be a string between 2 and 80 characters",
        );
      }
...
router.patch(
  "/me/preferences",
  asyncHandler(async (req, res) => {
    const { genres, language } = req.body as {
      genres?: string[];
      language?: string;
    };
    ...
    if (genres !== undefined) {
      if (!Array.isArray(genres) || genres.length > 5) {
        throw new ApiError(
          400,
          "genres must be an array with at most 5 entries",
        );
      }
      if (
        genres.some((genre) => typeof genre !== "string" || genre.length > 50)
      ) {
        throw new ApiError(
          400,
          "each genre must be a string up to 50 characters",
        );
      }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [user, byStatusRaw, favouriteGenreRaw, longestSessionRaw] =
      await Promise.all([
